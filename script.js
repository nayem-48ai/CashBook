document.addEventListener('DOMContentLoaded', function () {
    ...
    // === নতুন অ্যানিমেশন সহ 'Add Category' ফাংশন ===
    function addCategory() {
        const btn = document.getElementById('add-category-btn');
        const input = document.getElementById('newCategoryNameInput');
        const name = input.value.trim();
        if (!name) { showToast('Category name cannot be empty.', 'danger'); return; }
        if (categories.map(c => c.toLowerCase()).includes(name.toLowerCase())) { showToast('This category already exists.', 'danger'); return; }
    ...
    // === নতুন অ্যানিমেশন সহ 'Add Transaction' ফাংশন ===
    function addTransaction() {
        const btn = document.getElementById('save-transaction-btn');
        const category = document.getElementById('category').value;
        const amount = parseFloat(document.getElementById('amount').value);
        if (!category) { showToast('Please choose a category.', 'danger'); return; }
        if (isNaN(amount) || amount <= 0 || !document.getElementById('date').value) { showToast('Please enter a valid amount and date.', 'danger'); return; }
        const originalText = btn.innerHTML;
        btn.disabled = true;
        btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span> Saving...`;
        setTimeout(() => {
            allCashbooks[currentCashbookId].transactions.push({type: document.getElementById('transactionType').value, amount, category, date: document.getElementById('date').value, notes: document.getElementById('notes').value.trim()});
            saveData();
            renderTransactions();
    ...
    function printCashbook() { const book = allCashbooks[currentCashbookId]; const printContainer = document.getElementById('print-container'); const { totalIn, totalOut, balance } = calculateTotals(book.transactions); let rows = [...book.transactions].sort((a, b) => new Date(a.date) - new Date(b.date)).map(t => `<tr><td>${new Date(t.date).toLocaleDateString('bn-BD')}</td><td>${t.category}</td><td>${t.type === 'in' ? 'Cash In' : 'Cash Out'}</td><td style="text-align: right;">${t.amount.toFixed(2)}</td></tr>`).join(''); const printContent = `<div style="font-family: Arial, sans-serif; width: 90%; margin: auto;"><h1 style="text-align: center; color: #0d6efd;">Cashbook: ${book.name}</h1><p style="text-align: center; color: #6c757d;">Report Generated on: ${new Date().toLocaleString('bn-BD')}</p><hr><table style="width: 100%; border-collapse: collapse;"><thead style="background-color: #f2f2f2;"><tr><th style="padding: 10px; border: 1px solid #ddd; text-align: left;">তারিখ</th><th style="padding: 10px; border: 1px solid #ddd; text-align: left;">ক্যাটাগরি</th><th style="padding: 10px; border: 1px solid #ddd; text-align: left;">ধরন</th><th style="padding: 10px; border: 1px solid #ddd; text-align: right;">পরিমাণ</th></tr></thead><tbody>${rows}</tbody><tfoot style="font-weight: bold;"><tr><td colspan="3" style="text-align: right; padding-top: 15px;">Total Cash In:</td><td style="text-align: right; color: green; padding-top: 15px;">${totalIn.toFixed(2)}</td></tr><tr><td colspan="3" style="text-align: right;">Total Cash Out:</td><td style="text-align: right; color: red;">${totalOut.toFixed(2)}</td></tr><tr><td colspan="3" style="text-align: right; border-top: 1px solid #ddd; padding-top: 5px;">Balance:</td><td style="text-align: right; border-top: 1px solid #ddd; padding-top: 5px; color: ${balance >= 0 ? 'green' : 'red'};">${balance.toFixed(2)}</td></tr></tfoot></table><footer style="text-align: center; margin-top: 30px; padding-top: 15px; border-top: 1px solid #ddd; font-size: 12px;">all Rights reserve © Tnayem48</footer></div>`; printContainer.innerHTML = printContent; window.print(); printContainer.innerHTML = ''; }
